...
const catalog = [
  {
    scheme: `${BASE}/photos/*`,
    endpoint: `${BASE}/oembed`,
    types: ['photo', 'video', 'rich', 'link']
  }
];

function oembedData(reqUrl, maxwidth, maxheight) {
...
<ul>
  ...
  if (path === '/providers.json') {
    const body = JSON.stringify(
      catalog.map(c => ({
        provider_name: 'Lumovy',
        provider_url: BASE,
        endpoints: [{ schemes: [c.scheme], url: c.endpoint, formats: ['json', 'xml'] }]
      })),
      null,
      2
    );
  ...
  if (path === '/oembed') {
    ...
    const data = oembedData(target, maxwidth, maxheight);
...
server.listen(PORT, HOST, () => {
  console.log(`Lumovy oEmbed provider running at ${BASE}`);
});